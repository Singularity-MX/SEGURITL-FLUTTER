...
class AgregarInfoGuardia extends StatefulWidget {
  
    final String userId;

  AgregarInfoGuardia({required this.userId});

  @override
  _AgregarInfoGuardiaState createState() => _AgregarInfoGuardiaState();
}

class _AgregarInfoGuardiaState extends State<AgregarInfoGuardia> {
  File? _image1;
 File? _image2;
  ...
  Future<void> addExpediente(String formData) async {
    try {
      final response = await http.post(
        Uri.parse(backendUrl + '/api/users/expediente/add'),
        headers: {
              ...
              child: Column(
                children: [
                  ...
                  Container(
                    width: double.infinity,
                    child: Text(
                      'Ingresa la información del guardia',
                      style: TextStyle(
                        fontSize: 16,
...
// Vista previa de la imagen
                  Container(
                    ...
                    height: MediaQuery.of(context).size.width *
                        0.8, // Ajusta la altura según tus necesidades
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(10),
                      color: Colors.white,
                      boxShadow: [
                    ...
                    try {
                      ...
                      if (curp.isEmpty) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content:
                                Text('Por favor, ingresa el curp '),
                          ),
                        );
                        return; // Salir de la función sin continuar
                      }
                      ...
                      if (_image1 == null) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content:
                                Text('Por favor, toma la fotografía del guardia'),
                          ),
                        );
                        return; // Salir de la función sin continuar
                      }
                      ...
                      if (_image2 == null) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content:
                                Text('Por favor, toma la fotografía de su credencial'),
                          ),
                        );
                        return; // Salir de la función sin continuar
                      }
