...
class VistaGuardia extends StatefulWidget {
  final dynamic informacion;

  VistaGuardia({required this.informacion});

  @override
  _VistaGuardiaState createState() => _VistaGuardiaState();
}

class _VistaGuardiaState extends State<VistaGuardia> {
  File? _image1;
  File? _image2;
  ...
  void initState() {
    ...
    getInformacion();
    //obten los valores
    String email = widget.informacion['email'];
  }

  Future<void> getInformacion() async {
    try {
      final response = await http.get(Uri.parse(ApiConfig.backendUrl +
          '/api/users/expediente/getID/' +
  ...
  Widget build(BuildContext context) {
    ...
    String rutaImagen = ApiConfig.backendUrl + "/" + JSON['foto_url'];
    String rutaImagen2 =
        ApiConfig.backendUrl + "/" + JSON['fotocredencial_url'];
    print(rutaImagen);
    return Scaffold(
